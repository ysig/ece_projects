...
int read_msg(char buf[100],int newsd){
ssize_t n;
n = read(newsd, buf, sizeof(buf));
return n;
}
int send_msg(int f,int newsd,char A[100]){
		...
		for(i = 0; i <= fdmax; i++) {
			if (FD_ISSET(i, &read_fds)) { // we got one!!
				...
				if(peer[i].init==0){
					...
					if (send(j, strdup("Do you to speak with someone of the following?\n"), strlen(strdup("Do you to speak with someone of the following?\n")), 0) == -1) {
                                        		perror("send");
                                	}				
...
client_data:			  }	// END handle data from client
