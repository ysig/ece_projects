<?php
  ...
  //search into db for that doctor 
   if($result){
      if ($row = mysqli_fetch_row($result)){ // an uparxei farmakeio me tetoio ID

         $query= "SELECT * FROM contract WHERE PharmacyId=$Id ";
         $result1= mysqli_query($link,$query);
         $query1= "SELECT * FROM sell WHERE PharmacyId=$Id "  ;
         $res1= mysqli_query($link,$query1);
         if($result1 || $res1){
                if($full= mysqli_fetch_row($result1) || $full1= mysqli_fetch_row($res1)){ 
                     header('Location: Pharmacy-Delete-WithSell.html'); 
              #    echo "There are patients that are connected to Doctor with Id=$Id. Pressing submit you will also delete all of them." ;
                   ...
                   $query="DELETE FROM pharmacy WHERE PharmacyId=$Id ";
          ...
          header('Location: Pharmacy-Delete-WrongId.html');
   } 
   else
      echo "DB ERROR2!";        
