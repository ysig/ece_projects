<?php
  ...
  //define variables
   ...
   $Id2=$_POST["PharmaceuticalCompanyId"];

   $query= "SELECT * FROM contract WHERE (PharmacyId=$Id1 AND PharmaceuticalCompanyId=$Id2 )"  ; 
   $result= mysqli_query($link,$query);
   $redirect="Contract-Delete.html"; 
//   $_SESSION['varname'] = $Id;

  //search into db for that doctor 
   if($result){
      if ($row = mysqli_fetch_row($result)){ // an uparxei suntagh me tetoio ID
                   $query="DELETE FROM contract WHERE PharmacyId=$Id1 AND PharmaceuticalCompanyId=$Id2";
          ...
          header('Location: Contract-Delete-WrongId.html');
   } 
   else
      echo "DB ERROR2!";        
