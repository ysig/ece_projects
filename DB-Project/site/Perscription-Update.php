	<?php	
		...
		#define vars
		$pkargs = array($_GET['PatientId'],$_GET['DoctorId'],$_GET['DrugId']);
		$args = array(date('Y-m-d H:i:s', strtotime($_GET["Date"])),$_GET['Quantity']);
		$pkfields = array('`PatientId`','`DoctorId`','`DrugId`');
		$fields = array('`Date`','`Quantity`');
		$redirect = array('index.html','Perscription-Update.html');
		$table = '`prescription`';
